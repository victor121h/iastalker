...
async function sendAbandonedCartEmail(customerEmail: string, customerName: string) {
  try {
    ...
    await client.emails.send({
      ...
      to: customerEmail,
      subject: 'AI Ghost at half price — discover everything now...',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff; color: #333333; padding: 40px 30px;">
          <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #333333; font-size: 24px; margin-bottom: 8px;">AI Ghost at Half Price</h1>
            <p style="color: #666666; font-size: 14px;">Discover everything now...</p>
          </div>

          <p style="font-size: 15px; line-height: 1.8; color: #444444;">You just earned a <strong style="color: #dc2626;">50% discount</strong> on AI Ghost. Discover everything for only <strong style="color: #10b981;">$14.90</strong>!</p>

          <div style="text-align: center; margin: 30px 0;">
            <a href="https://go.centerpag.com/PPU38CQ8ABS" style="display: inline-block; background-color: #10b981; color: #ffffff; font-weight: bold; font-size: 15px; padding: 14px 32px; border-radius: 6px; text-decoration: none;">Get My Discount</a>
          </div>
          ...
          <div style="text-align: center;">
            ...
            <p style="font-size: 11px; color: #999999;">You are receiving this email because you started a purchase on our platform.</p>
    ...
    console.log('[PerfectPay Webhook] Abandoned cart email sent to:', customerEmail);
  } catch (emailError: any) {
    console.error('[PerfectPay Webhook] Failed to send abandoned cart email:', emailError?.message || emailError);
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const isAbandoned = saleStatus === 4;
    if (isAbandoned && customerEmail) {
